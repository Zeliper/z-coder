...
Claude Code를 위한 범용 멀티 에이전트 오케스트레이션 시스템입니다. Claude Code의 백그라운드 에이전트(Async Sub-agent) 기능을 활용하여 `/orchestrate` 커맨드가 서브 에이전트들을 병렬로 spawn하고 조율합니다.

[English Documentation](./README.md)

## 주요 기능

- **멀티 에이전트 오케스트레이션**: `/orchestrate` 커맨드가 여러 서브 에이전트를 병렬로 조율
...
| Skill | 용도 |
|-------|------|
| `orchestration-workflow` | /orchestrate 커맨드용 메인 워크플로우 가이드 |
